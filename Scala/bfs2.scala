object Main extends App {
	...
	val connected = Array.fill(n)(Vector[Int]())
	loop(0, m) { _ =>
		val a, b = Scanner.nextInt - 1
		connected(a) = connected(a) :+ b
		connected(b) = connected(b) :+ a
	}
	...
	while (queue.nonEmpty) {
		val q = queue.dequeue()
		visited(q) = true
		connected(q).filter{ v => !visited(v) && mark(v) == -1 } foreach { v =>
			mark(v) = q
			queue.enqueue(v)
		}
	}
	...
	pw.println("Yes")
	loop(1, n) { i => pw.println(mark(i) + 1) }
	pw.flush()
}
